...
var desPlaintext = []byte{1, 2, 3, 4, 5, 6, 7, 8}
var desCiphertext = []byte{185, 73, 135, 249, 137, 1, 122, 247}
...
func TestPbDecrypterFor(t *testing.T) {
	...
	_, _, err := pbBlockModeFor(alg, pass, false /*decrypt*/)
	if _, ok := err.(NotImplementedError); !ok {
		t.Errorf("expected not implemented error, got: %T %s", err, err)
	}

	alg.Algorithm = sha1WithTripleDES
	cbc, blockSize, err := pbBlockModeFor(alg, pass, false /*decrypt*/)
	if err != nil {
		t.Errorf("unexpected error from pbBlockModeFor %v", err)
	}
	if blockSize != 8 {
		t.Errorf("unexpected block size %d, wanted 8", blockSize)
	}

	ciphertext := make([]byte, len(desPlaintext))
	cbc.CryptBlocks(ciphertext, desPlaintext)

	if bytes.Compare(ciphertext, desCiphertext) != 0 {
		t.Errorf("bad ciphertext, got %x but wanted %x", ciphertext, desCiphertext)
	}
}

func TestPbEncryptorFor(t *testing.T) {
	params, _ := asn1.Marshal(pbeParams{
		Salt:       []byte{1, 2, 3, 4, 5, 6, 7, 8},
		Iterations: 2048,
	})
	...
	pass, _ := bmpString("Sesame open")
	cbc, blockSize, err := pbBlockModeFor(alg, pass, true /*encrypt*/)
	if err != nil {
		t.Errorf("unexpected error from pbBlockModeFor %v", err)
	}
	if blockSize != 8 {
	...
	plaintext := make([]byte, len(desCiphertext))
	cbc.CryptBlocks(plaintext, desCiphertext)

	if bytes.Compare(plaintext, desPlaintext) != 0 {
		t.Errorf("bad plaintext, got %x but wanted %x", plaintext, desPlaintext)
	}

	// The weak cipher must be rejected
	alg.Algorithm = oidPBEWithSHAAnd40BitRC2CBC
	cbc, blockSize, err = pbBlockModeFor(alg, pass, true /*encrypt*/)
	if _, ok := err.(NotImplementedError); !ok {
		t.Errorf("expected not implemented error for weak cipher, got: %T %s", err, err)
	}
}
...
func TestPbDecrypt(t *testing.T) {
	for i, test := range pbDecryptTests {
		...
		if test.expectedError == nil {
			ciphertext, err := pbEncrypt(decryptable.Algorithm(), test.expected, password)
			if err != nil {
				t.Errorf("#%d: unexpected encryption error %q", i, err)
				continue
			}

			if !bytes.Equal(ciphertext, test.in) {
				t.Errorf("#%d: got plaintext %x, but wanted %x", i, ciphertext, test.in)
			}
		}
	}
}
